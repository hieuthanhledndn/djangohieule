...
def  HOME(request):
    ...
    product_deal_of = Product.objects.filter(deal_of_the_day=True).order_by('-id')
    product_new_arrivals = Product.objects.filter(new_arrivals=True).order_by('-id')[:8]
    product_best_sellers = Product.objects.filter(best_sellers=True).order_by('-id')[:8]
    product_sale_items = Product.objects.filter(sale_items=True).order_by('-id')[:8]
